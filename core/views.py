...
def register_view(request):
    ...
    if request.user.is_authenticated:
        logger.info(f"🔄 [AUTH] User {request.user.username} sudah login -> Redirect Home.")
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if errors:
                logger.warning(f"⚠️ [REGISTER FAIL] ip={ip} validasi_error={errors}")
                return inertia_render(request, "Auth/Register", props={"errors": errors})

            user = User.objects.create_user(username=username, email=email, password=password)
            login(request, user)

            logger.info(f"👤 [REGISTER SUCCESS] user={user.username} id={user.id} ip={ip}")
            return redirect("home")

        except IntegrityError:
            logger.warning(f"⚠️ [REGISTER FAIL] ip={ip} username='{username}' sudah terpakai.")
            ...
            logger.error(f"❌ [REGISTER ERROR] ip={ip} err={e}", exc_info=True)
            return inertia_render(
                request,
                "Auth/Register",
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if user is not None:
                ...
                logger.warning(f"⚠️ [LOGIN FAIL] username={username} ip={ip}")
                return inertia_render(
                    request,
                    "Auth/Login",
            ...
            logger.error(f"❌ [LOGIN ERROR] ip={ip} err={e}", exc_info=True)
            return inertia_render(
                request,
                "Auth/Login",
...
def logout_view(request):
    ...
    if request.user.is_authenticated:
        ...
        ip = _get_client_ip(request)
        logout(request)
        logger.info(f"🚪 [LOGOUT] user='{user_name}' ip={ip} berhasil keluar.")
...
def chat_view(request):
    ...
    try:
        logger.info(f"🧠 [VIEW START] chat_view user={user.username}(id={user.id}) ip={ip}")
        ...
        history_data = [
            {
                ...
                "date": h.timestamp.strftime("%Y-%m-%d"),  # untuk UI grouping per hari
            }
            for h in histories
        ]
        ...
        logger.critical(
            f"❌ [VIEW ERROR] chat_view CRASH user={user.username}(id={user.id}) ip={ip} err={str(e)}",
            exc_info=True,
        )
        try:
...
def documents_api(request):
    ...
    if request.method != "GET":
        logger.warning(f"⚠️ [DOCS API] Method not allowed method={request.method} user={user.username}(id={user.id}) ip={ip}")
    ...
    try:
        ...
        dur = round(time.time() - t0, 4)
        logger.error(
            f"❌ [DOCS API ERROR] user={user.username}(id={user.id}) ip={ip} in {dur}s err={str(e)}",
            exc_info=True,
        )
...
def upload_api(request):
    ...
    if request.method == "POST":
        ...
        if not files:
            logger.warning(f"⚠️ [UPLOAD] submit tanpa file user={user.username}(id={user.id}) ip={ip}")
        ...
        for index, file_obj in enumerate(files):
            ...
            logger.debug(
                f"   ➡️ [UPLOAD ITEM] ({index+1}/{len(files)}) name='{file_obj.name}' size={file_size_kb}KB user={user.username}"
            )
            ...
            try:
                ...
                if success:
                    ...
                    # Jika gagal parsing, bersihkan DB record & file
                    doc_id = doc.id
                    doc.delete()
                    error_count += 1
                ...
                logger.error(
                    f"      ❌ [SYSTEM ERROR] file='{file_obj.name}' user={user.username}(id={user.id}) err={str(e)}",
                    exc_info=True,
                )
...
def chat_api(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            ai_start = time.time()
            answer = ask_bot(user.id, query)
            ai_dur = round(time.time() - ai_start, 2)

            # Guard jika answer bukan string (jaga-jaga)
            ...
            total_dur = round(time.time() - t0, 3)
            logger.error(
                f"❌ [CHAT CRASH] user={user.username}(id={user.id}) ip={ip} in {total_dur}s err={str(e)}",
                exc_info=True,
            )
