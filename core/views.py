...
def register_view(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if errors:
                logger.warning(f"⚠️ [REGISTER FAIL] ip={ip} errors={errors}", extra=_log_extra(request))
            ...
            logger.warning(f"⚠️ [REGISTER FAIL] ip={ip} username='{username}' already used", extra=_log_extra(request))
...
def login_view(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            logger.warning(f"⚠️ [LOGIN FAIL] username={username} ip={ip}", extra=_log_extra(request))
...
def documents_api(request):
    ...
    if request.method != "GET":
        logger.warning(f"⚠️ [DOCS API] Method not allowed method={request.method} ip={ip}", extra=_log_extra(request))
...
def upload_api(request):
    ...
    if request.method != "POST":
        logger.warning(f"⚠️ [UPLOAD] Method not allowed method={request.method} ip={ip}", extra=_log_extra(request))
        return JsonResponse({"status": "error", "msg": "Method not allowed"}, status=405)

    files = request.FILES.getlist("files")
    if not files:
        logger.warning(f"⚠️ [UPLOAD] submit tanpa file user={user.username}(id={user.id}) ip={ip}", extra=_log_extra(request))
    ...
    try:
        payload = service.upload_files_batch(user=user, files=files)
...
def chat_api(request):
    ...
    if request.method != "POST":
        logger.warning(f"⚠️ [CHAT] Method not allowed method={request.method} ip={ip}", extra=_log_extra(request))
        return JsonResponse({"status": "error", "msg": "Method not allowed"}, status=405)
    ...
    try:
        try:
            ...
            logger.warning(f"⚠️ [CHAT] Invalid JSON user={user.username}(id={user.id}) ip={ip}", extra=_log_extra(request))
            return JsonResponse({"error": "Invalid JSON"}, status=400)

        query = data.get("message")
        if not query:
            logger.warning(f"⚠️ [CHAT] Pesan kosong user={user.username}(id={user.id}) ip={ip}", extra=_log_extra(request))
        ...
        payload = service.chat_and_save(user=user, message=query, request_id=_rid(request))
...
def reingest_api(request):
    ...
    if request.method != "POST":
        logger.warning(f"⚠️ [REINGEST] Method not allowed method={request.method} ip={ip}", extra=_log_extra(request))
        return JsonResponse({"status": "error", "msg": "Method not allowed"}, status=405)
